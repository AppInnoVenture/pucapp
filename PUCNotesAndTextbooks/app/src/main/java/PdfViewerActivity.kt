...
class PdfViewerActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        onBackPressedDispatcher.addCallback(this) {
            if (isDownloading){
                DownloadService.stopDownload(this@PdfViewerActivity)
            ...
            if (!pdfOpened) {
                try {
                    ...
                    pdfView.fromAsset(pdfFilename).nightMode(isNight).fitEachPage(true)
                        ...
                        .defaultPage(sharedPreferences.getInt(pdfFileName, 0)).onLoad { nbPages ->
                            totalPgPdf = nbPages
                    ...
                    pdfView.fromFile(pdfFile).nightMode(isNight).fitEachPage(true)
                        .defaultPage(sharedPreferences.getInt(prefix, 0))
                        .scrollHandle(createScrollHandle()).password(REDACTED).onError { throwable ->
                            handlerPdf(throwable = throwable)
                        }.swipeHorizontal(isSwipeHorizontal).spacing(spacing).onLoad { nbPages ->
                            totalPgPdf = nbPages
                            if (curSlideshow) startSlideshow(slideInterval.toDouble())
                            else if (curAutoScrolling) startAutoScroll()
                        }.pageSnap(isPageSnap).pageFling(isPageFling).load()
    ...
    private fun openAnsPdf() {
            ...
            if (!anspdfOpened) {
                try {
                    pdfView.fromAsset(ansPdf).nightMode(isNight)
                        .defaultPage(sharedPreferences.getInt("ans$prefix", 0))
                        .fitEachPage(true)
                        .scrollHandle(createScrollHandle()).password(REDACTED)
                        .swipeHorizontal(isSwipeHorizontal).onLoad { nbPages ->
