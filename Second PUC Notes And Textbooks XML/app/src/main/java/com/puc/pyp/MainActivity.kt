...
class MainActivity : BaseActivity() {
        ...
        registerForActivityResult(
            ...
            if (!isGranted && Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                basePreferences.edit {
                    putBoolean("notification_permission_permanently_denied",
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (isDarkMode)
            binding.navView.menu.findItem(R.id.nav_dark).title = getString(R.string.nav_light)
...
//                }
                ...
                R.id.nav_dark -> {
                    drawerLayout.closeDrawer(GravityCompat.START)
                    basePreferences.edit(commit = true) { putBoolean("dark_mode", !isDarkMode) }
                    recreate()
                }
        ...
        drawerLayout.post {
            onBackPressedDispatcher.addCallback(this, backCallback.apply {
                isEnabled = drawerLayout.isDrawerOpen(GravityCompat.START)
            })
        }
    ...
    companion object {
        ...
        private val languageNames by lazy { LANGUAGES.map { it.displayName }.toTypedArray() }
    }

    private fun showLanguageSelectionDialog() {
        val currentLanguage = basePreferences.getString("language", "sys") ?: "sys"
        var selectedLanguageCode: String = currentLanguage

        val checkedItem = LANGUAGES.indexOfFirst { it.code == currentLanguage }.coerceAtLeast(0)

        MaterialAlertDialogBuilder(this)
            .setTitle(R.string.nav_kan)
            .setSingleChoiceItems(languageNames, checkedItem) { _, which ->
                selectedLanguageCode = LANGUAGES[which].code
            }
            .setPositiveButton(R.string.dialog_button_ok) { _, _ ->
                if (selectedLanguageCode != currentLanguage) {
                    basePreferences.edit { putString("language", selectedLanguageCode) }
                    recreate()
                }
            }
